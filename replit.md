...
- **Bed Conflict Resolution (v1.5.8.1)**: Before inserting or reactivating a patient, the system checks if the target bed is occupied by a different patient (different codigoAtendimento). If so, the old patient is archived as "registro_antigo" and soft-deleted (leito changed to `ARCHIVED_{leito}_{timestamp}`, status set to `archived`) to release the UNIQUE constraint while preserving referential integrity with notifications and audit events. Uses `getPatientByLeito()` method for conflict detection.
